...
class AutoRallyCtrlr(object):
  ...
  def __init__(self, namespace='controller_manager'):
      ...
      self.chassisCmds = chassisCommand() # dict()
      ...
      self.chassisCmdSub = rospy.Subscriber(self._vehicle_prefix+"/chassisCommand", chassisCommand,
    ...
    while not rospy.is_shutdown():
      ...
      if (self._cmd_timeout > 0.0 and
        ...
        with self.chassisCmdLock:
          ...
          if abs(self.chassisCmds.steering) <= 1.0 and \
              (rospy.Time.now()-self.chassisCmds.header.stamp) < \
                rospy.Duration.from_sec(0.2) and\
              not foundSteering:
            #rospy.loginfo("%s in control of steering", cmd);
            steer_ang = -math.radians(25)*self.chassisCmds.steering
            steer_ang_vel = 0.0
            chassisSt.steering = self.chassisCmds.steering
            chassisSt.steeringCommander = self.chassisCmds.sender
            foundSteering = True

          if abs(self.chassisCmds.throttle) <= 1.0 and \
              (rospy.Time.now()-self.chassisCmds.header.stamp) < \
                rospy.Duration.from_sec(0.2) and\
              not foundThrottle:

            #rospy.loginfo("%s in control of throttle", cmd);
            if self.chassisCmds.throttle >= 0.0:
              speed = self.rear_axle_max_effort*self.chassisCmds.throttle
            else:
              speed = self.rear_axle_brake_effort*self.chassisCmds.throttle

            accel = 0.0
            chassisSt.throttle = self.chassisCmds.throttle
            chassisSt.throttleCommander = self.chassisCmds.sender
            foundThrottle = True


          if self.chassisCmds.frontBrake >= 0.0 and \
              self.chassisCmds.frontBrake <= 1.0 and \
              (rospy.Time.now()-self.chassisCmds.header.stamp) < \
                rospy.Duration.from_sec(0.2) and\
              not foundFrontBrake:
  ...
  def chassisCmdCb(self, chassisCommand):
    ...
    with self.chassisCmdLock:
      #self.lastCmdTime = rospy.get_time()
      self.chassisCmds = chassisCommand
      self.chassisCmds.header.stamp = rospy.Time.now()
      self._last_cmd_time = rospy.get_time()
